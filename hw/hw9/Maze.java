...
//           	sumWalls+=walls[m][2];
    ...
    //create a counter
    int w = walls.length;
//    System.out.println(sumWalls);
    
    while(w>1) {
    	//randomly choose a wall
    	int randWall = randInt(w);
    	//and swap it to the end
    	int[] temp = walls[w-1];
    	walls[w-1] = walls[randWall];
    	walls[randWall] = temp;
    	w--;
    }
    ...
    for(int[] wall: walls) {
    	...
    	int set1, set2;
    	if(wall[3]==0) {//horizontal
    		set1 = cells.find(wall[0]+wall[1]*horiz);
    		set2 = cells.find(wall[0]+(wall[1]+1)*horiz);
    		if(set1!=set2) {
    			wall[2]=0;
    			hWalls[wall[0]][wall[1]]=false;
    			cells.union(set1, set2);
        	}
    	}else {//vertical 
    		set1 = cells.find(wall[0]+wall[1]*horiz);
   		 	set2 = cells.find(wall[0]+1+wall[1]*horiz);
	   		if(set1!=set2) {
	   			wall[2]=0;
				vWalls[wall[0]][wall[1]]=false;
				cells.union(set1, set2);
	   		}
