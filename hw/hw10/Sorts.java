...
public class Sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    // Replace the following line with your solution.
	  ...
	  int[] sortedOn = new int[keys.length];
	  for(int i = 0; i < keys.length; i++) {
		  String s = Integer.toString(keys[i], 16); //key in String (16 bit)
		  s = String.format("%8s",s).replace(" ", "0");// add padding 0s
		  String d = s.substring(whichDigit, whichDigit+1); //digit to be sorted on 
		  int kk = Integer.parseInt(d,16); //digit in int
		  sortedOn[i] = kk; //store the key for each int
		  counts[kk]++; 
	  }
	  ...
	  for(int k = 0; k < keys.length; k++) {
		  sorted[counts[sortedOn[k]]] = keys[k];
		  counts[sortedOn[k]]++;
	  }
